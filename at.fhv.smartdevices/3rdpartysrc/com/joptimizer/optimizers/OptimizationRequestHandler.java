...
public abstract class OptimizationRequestHandler {
	...
	// double[] x = X.toArray();
	// for(int i=0; i<request.getFi().length; i++){
	// ret = F2.appendRows(ret, ALG.multOuter(F1.make(1, 1.),
	// F1.make(request.getFi()[i].gradient(x)), null));
	// }
	// return ret;
	// }
	...
	/**
	 * Overriding this, a subclass can define some extra condition for exiting
	 * the iteration loop.
	 */
