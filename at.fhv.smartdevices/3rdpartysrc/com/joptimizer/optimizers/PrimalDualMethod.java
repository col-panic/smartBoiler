...
public class PrimalDualMethod extends OptimizationRequestHandler {
	...
	public int optimize() throws Exception {
		...
		if (L0 != null) {
			...
			L0 = F1.make(getFi().length, Math.min(1, (double) getDim() / getFi().length));// must
																							// be
																							// >0
		}
		if (log.isDebugEnabled()) {
		...
		while (true) {
			...
			if (isCheckProgressConditions()) {
				if (!Double.isNaN(previousRPriXNorm) && !Double.isNaN(previousRDualXLVNorm) && !Double.isNaN(previousSurrDG)) {
			...
			for (int j = 0; j < getFi().length; j++) {
				...
				// log.debug("HessSum    : " +
				// ArrayUtils.toString(HessSum.toArray()));
			}

			// DoubleMatrix2D GradSum = F2.make(getDim(), getDim());
			// for (int j = 0; j < getFi().length; j++) {
			// DoubleMatrix1D g = GradFiX.viewRow(j);
			// GradSum.assign(ALG.multOuter(g, g,
			// null).assign(Mult.mult(-L.get(j) / fiX.get(j))), Functions.plus);
			// //log.debug("GradSum    : " +
			// ArrayUtils.toString(GradSum.toArray()));
			// }
			DoubleMatrix2D GradSum = F2.make(getDim(), getDim());
			for (int j = 0; j < getFi().length; j++) {
			...
			// DoubleMatrix2D Hpd = getHessF0(X).assign(HessSum,
			// Functions.plus).assign(GradSum, Functions.plus);
			...
			while (cnt < 500) {
				...
				// X1.assign(stepX.copy().assign(Mult.mult(s)).assign(X,
				// Functions.plus));
				// L1.assign(stepL.copy().assign(Mult.mult(s)).assign(L,
				// Functions.plus));
				// V1.assign(stepV.copy().assign(Mult.mult(s)).assign(V,
				// Functions.plus));
				...
				if (isInDomainF0(X1)) {
					...
					double normRX1L1V1t = Math.sqrt(ALG.norm2(rPriX1) + ALG.norm2(rCentX1L1t) + ALG.norm2(rDualX1L1V1));
			...
			// normRXLVt = Math.sqrt(Math.pow(rPriXNorm, 2) +
			// Math.pow(rCentXLtNorm, 2) + Math.pow(rDualXLVNorm, 2));
			// if(log.isDebugEnabled()){
			// log.debug("rPri  norm: " + rPriXNorm);
			// log.debug("rCent norm: " + rCentXLtNorm);
			// log.debug("rDual norm: " + rDualXLVNorm);
