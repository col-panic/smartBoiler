...
public class Complex {
   ...
   public static double abs( double[] z ) {
      ...
      if ( abs(z[0]) == 0 && abs(z[1]) == 0 ) {
         x = 0.0;
      } else if ( abs(z[0]) >= abs(z[1]) ) {
         h = z[1]/z[0];
         x = abs( z[0] ) * sqrt(1 + h*h);
      } else {
         h = z[0]/z[1];
         x = abs( z[1] ) * sqrt( 1 + h*h );
      } 
      return x;
   }
   ...
   public static double[] exp( double[] z) {
      ...
      double[] w = {exp(z[0]) * cos(z[1]), exp(z[0]) * sin(z[1]) };
      return w;
   }
   ...
   public static double[] gamma(double[] z) {
      ...
      if (reflec) {
         ...
         if (abs(x[1]) > 709) {  // sin( 710 i ) = Infinity !! 
            return ZERO_;
         }
   ...
   public static double[] lnSin( double[] z ) {
      ...
      if ( abs(z[1]) <= 709 ) {
         result[0] = sin(z[0]) * cosh(z[1]);  // since JDK 1.5:
         //result[0] = sin(z[0]) * ( exp(z[1]) + exp(-z[1]) ) / 2;
         result[1] = cos(z[0]) * sinh(z[1]); // since JDK 1.5
   ...
   public static double[] power( double x, double[] s ) {
      ...
      w[1] = w[0];
      if ( x > 0 ) {
         w[0] *= cos( s[1] * log( absX ) );
         w[1] *= sin( s[1] * log( absX ) );
      } else {
         w[0] *= cos( s[1] * log( absX ) + s[0]*PI );
         w[1] *= sin( s[1] * log( absX ) + s[0]*PI );
      }
   ...
   public Complex pow( Complex s ) {
      ...
      double[] w = new double[2];
      w[0] = Math.pow( r, s.z[0] ) * exp( - s.z[1] * phi );
      w[1] = w[0];
   ...
   public static double[] power( double[] z, double[] s ) {
      ...
      double[] w = new double[2];
      w[0] = Math.pow( r, s[0] ) * exp( - s[1] * phi );
      w[1] = w[0];
   ...
   public static Complex sin(Complex z) {
      return new Complex(
         sin(z.z[0]) * cosh(z.z[1]),
         cos(z.z[0]) * sinh(z.z[1])
      );
   }
   ...
   public static double[] sqrt( double[] z ) {
      ...
      if ( abs(z[0]) != 0 || abs(z[1]) != 0 ) {      
         if ( abs(z[0]) >= abs(y[1]) ) {
            h = z[1]/z[0];
            w = sqrt( ( 1 + sqrt( 1 + h*h ) ) / 2 );
            w = sqrt(z[0]) * w;
         } else {
            h = abs(z[0]/z[1]);
            w = sqrt( ( h + sqrt( 1 + h*h ) ) / 2 );
            w = sqrt(z[1]) * w;
         }
   ...
   public static String toString(double[] z, java.text.DecimalFormat digit) {
      ...
      if ( Double.toString(z[0]).equals("NaN") || Double.toString(z[1]).equals("NaN") ) {
         output += "NaN";
      } else if ( abs(z[0]) < ACCURACY && abs(z[1]) < ACCURACY ) { //z[0] == 0 && z[1] == 0
         output += "0";
      } else if ( abs(z[0]) >= ACCURACY && abs(z[1]) < ACCURACY ) {
         formatCondition = ( abs(z[0]) < upLimit && abs(z[0]) > lowLimit );
         output += formatCondition ? digit.format(z[0]) : scientific.format(z[0]);
      } else if ( abs(z[0]) >= ACCURACY && z[1] > 0 ) {
         formatCondition = ( abs(z[0]) < upLimit && abs(z[0]) > lowLimit );
         output += formatCondition ? digit.format(z[0]) : scientific.format(z[0]);
         output += " + "; 
         if ( abs(z[1] - 1) >= ACCURACY ) {
            formatCondition = ( abs(z[1]) < upLimit && abs(z[1]) > lowLimit );
            output += formatCondition ? digit.format(abs(z[1])) : scientific.format(z[1]);
            output += " ";
         }
         output += "i";
      } else if ( abs(z[0]) >= ACCURACY && z[1] < 0 ) {
         ...
         output += formatCondition ? digit.format(z[0]) : scientific.format(z[0]);
         output += " - ";
         if ( abs(z[1] + 1) >= ACCURACY ) { //z[1] != -1
            formatCondition = ( abs(z[1]) < upLimit && abs(z[1]) > lowLimit );
            output += formatCondition ? digit.format(abs(z[1])) : scientific.format(abs(z[1]));
            output += " ";
         }
         output += "i";
