...
/**
 * @author kepe Tests for the class @link DemandCalculationModel All test use @see
 *         {@link DemandCalculationTests#testInverseModel(int, int, double, int, double)}
 * @class
 */
public class DemandCalculationTests {
	...
	/**
	 * Creates random demands and on/off switch times, simulates the dhwh and
	 * finally, calculates the demand by using the inverse model.
	 * 
	 * @param deltat
	 *            time step (ms)
	 * @param interpolationFactor
	 *            (nr of steps to interpolate per time step)
	 * @param maxDemand
	 *            max demand in W
	 * @param iterations
	 *            nr of time steps
	...
	private void testInverseModel(int deltat, int interpolationFactor, double maxDemand, int iterations, double error, double switchProb, float initialTemp) {
		...
		for (Long i = 0L; i < iterations; i++) {
			...
			double demand = 0;
			if (maxDemand > 0) {
				demand = random.nextDouble() > 0.3 ? random.nextDouble() * maxDemand / deltat * 1000 : 0;
			}
		...
		TreeMap<Long, Double> measureddemands = DemandCalculationModel.calculateDemand(switchMap, temp, deltat / interpolationFactor, false);
		TreeMap<Long, Double> measureddemandsOde = DemandCalculationModel.calculateDemand(switchMap, temp, deltat / interpolationFactor, true);
