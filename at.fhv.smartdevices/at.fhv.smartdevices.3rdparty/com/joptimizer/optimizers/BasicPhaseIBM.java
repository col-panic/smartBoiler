...
public class BasicPhaseIBM {
	...
	public double[] findFeasibleInitialPoint() throws Exception {
		...
		// ConvexMultivariateRealFunction[] inequalities = new
		// ConvexMultivariateRealFunction[originalProblem.getFi().length];
		// for(int i=0; i<inequalities.length; i++){
		//
		// final ConvexMultivariateRealFunction originalFi =
		// originalProblem.getFi()[i];
		//
		// ConvexMultivariateRealFunction fi = new
		...
		// DoubleMatrix2D[][] parts = new DoubleMatrix2D[][]{{origHess,
		// null},{null,F2.make(1, 1)}};
		...
		// if(originalFiX0.get(maxIneqIndex) +
		// originalProblem.getTolerance()<0){
		...
		// initialPoint.set(dim-1, Math.max(initialPoint.get(dim-1),
		// originalFiX0.get(i)*Math.pow(originalProblem.getToleranceFeas(),-2)));
		// }
