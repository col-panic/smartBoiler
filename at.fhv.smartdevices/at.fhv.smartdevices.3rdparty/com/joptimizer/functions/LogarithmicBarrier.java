...
public class LogarithmicBarrier implements BarrierFunction {
	...
	public double[][] hessian(double[] X) {
		...
		for (int j = 0; j < fi.length; j++) {
			...
			DoubleMatrix2D ineqHessjX = (fijHessianX != FunctionsUtils.ZEROES_2D_ARRAY_PLACEHOLDER) ? F2.make(fijHessianX)
					: FunctionsUtils.ZEROES_MATRIX_PLACEHOLDER;
			DoubleMatrix1D ineqGradjX = F1.make(fi[j].gradient(X));
	...
	/**
	 * Create the barrier function for the Phase I. It is a LogarithmicBarrier
	...
	/**
	 * Calculates the initial value for the s parameter in Phase I. Return s =
	 * max(fi(x))
	 * 
	...
	public double calculatePhase1InitialFeasiblePoint(double[] originalNotFeasiblePoint, double tolerance) {
		...
		// lucky strike?
		int maxIneqIndex = Utils.getMaxIndex(fiX0NF.toArray());
